...
const Edit = (props) => {
	...
	const isEditPrestation = useRouteMatch('/admin/prestation/:param').params.param === 'edit'
	const imgRef = useRef()
	const { PRESTATION ,CATEGORY } = useContext(DBContext)
	...
	const prestationID = props.location.state ? props.location.state.id : null
	...
	useEffect(() => {
		if (prestationID) {
			PRESTATION.findOneById(prestationID).then(result => {
				if (result) {
					setPrestation(result)
				} else {
					props.history.push('/admin/prestation/all')
				}
			}).catch(e => console.log(e.message || e))
		} else if (isEditPrestation) {
			props.history.push('/admin/prestation/all')
		} else {
			setPrestation(initialValue)
			setMessage(null)
		}
	}, [PRESTATION, prestationID, props.history, isEditPrestation])
	...
	const handleSubmit = e => {
		...
		PRESTATION.add(prestation).then(() => {
			!isEditPrestation && setPrestation(initialValue)
			setMessage({
				type: 'success',
	...
	const submitBtnDisabled = title === "" || image === "" || content === '' || price === '' || duration === '' || category === '';
	...
	return (
		<div className=" py-5 container">
			<div className="d-flex flex-wrap align-items-center">
			{
					isEditPrestation ? (
						<Fragment>
							<h1 className="h2 mr-3">Edit Prestation</h1>
			...
			<form className="form container" onSubmit={handleSubmit}>
						...
						<select type="text" value={category} onChange={handleChange} id="category" className="form-control">
							<option value=""> -- Select a category --</option>
							{
								categories.map(category => (
									<option key={category.id} value={category.slug} className="text-capitalize">{category.name}</option>
								))
							}
						</select>
					</div>
				...
				<button type="submit" className="btn btn-info" disabled={submitBtnDisabled}>Register Prestation</button>
